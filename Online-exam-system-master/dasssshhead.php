  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
      <ul class="nav navbar-nav">
        ...
        <li <?php if(@$_GET['q']==5) echo'class="active"'; ?>><a href="headdash.php?q=6">Add Student</a></li>
      </ul> 
          </div><!-- /.navbar-collapse -->
  </div><!-- /.container-fluid -->
</nav>
...
<tr ><td><b>Rank</b></td><td><b>Name</b></td><td><b>Gender</b></td><td><b>USN</b></td><td><b>Score</b></td></tr>';
$c=0;
...
$c++;
echo '<tr><td style="color:#99cc32"><b>'.$c.'</b></td><td>'.$name.'</td><td>'.$gender.'</td><td>'.$USN.'</td><td>'.$s.'</td><td>';
}
echo '</table></div>';}
...
<?php
if (@$_GET['q'] == 1) {
    // Ensure $con is initialized and points to a valid database connection
    if (!$con) {
        die("Database connection failed: " . mysqli_connect_error());
    }

    // Query the user table
    $result = mysqli_query($con, "SELECT * FROM user") or die('Error fetching users');
    ...
    $c = 1; // Initialize serial number counter

    // Fetch and display each row of the result set
    while ($row = mysqli_fetch_assoc($result)) {
        $name = htmlspecialchars($row['name']);
        $gender = htmlspecialchars($row['gender']);
        $USN = htmlspecialchars($row['college']); // Adjust if 'USN' column has a different name in your DB
        $email = htmlspecialchars($row['email']);
        $mob = htmlspecialchars($row['mob']);
